...
tokenControl = new FormControl('', [Validators.required, Validators.minLength(6),
  ...
  async signUp() {
    if (this.formSignUp().email.valid && this.formSignUp().password.valid) {
      ...
      if (error) {
        ...
        this.notify.show("Verification code sent to your email!");
  ...
  async resendVerification() {
    const email = this.formSignUp().email.value;
    if (!email) {
      this.notify.show('Please enter your email.');
      return;
    }
    ...
    if (success) {
      this.notify.show('New verification link sent! Check your inbox (and spam folder).');
    } else {
      this.notify.show('Failed to send verification link. Please try again.');
    }
  }
  async verifyOTP() {
  if (this.tokenControl.valid) {
    ...
    if (error) {
      this.notify.show('Invalid or expired code');
      console.error('OTP Error:', error.message);
    } else {
      this.notify.show('Email verified successfully!');
